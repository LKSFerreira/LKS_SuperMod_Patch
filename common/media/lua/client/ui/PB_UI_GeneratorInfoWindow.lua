...
local PB_GeneratorInfoWindow = ISCollapsableWindow:derive("PB_GeneratorInfoWindow")
PB_GeneratorInfoWindow.instances = {}
...
function PB_GeneratorInfoWindow:initialise()
    ISCollapsableWindow.initialise(self)
end

function PB_GeneratorInfoWindow:createChildren()
    ...
    self.coverageBtn = ISButton:new(
        ...
        self, PB_GeneratorInfoWindow.onToggleCoverage
    )
    ...
    self.closeBtn = ISButton:new(
        ...
        self, PB_GeneratorInfoWindow.onClose
    )
...
--- Retorna: { winW, barCol }
function PB_GeneratorInfoWindow:calculateLayout()
    local textMgr = getTextManager()
    local maxLabelW = 0

    local labels = {
        getText("IGUI_PB_PoolFuel") or "Combustível do Reservatório",
        getText("IGUI_PB_PowerDraw") or "Consumo Elétrico",
        getText("IGUI_PB_FuelRate") or "Consumo de Combustível",
        getText("IGUI_EstRuntime") or "Duração Estimada",
        getText("IGUI_SystemStrain") or "Carga da Rede Elétrica",
        getText("IGUI_PB_Consumers") or "Aparelhos Conectados",
        getText("IGUI_Lights") or "Luzes",
        getText("IGUI_PB_Lamps") or "Lâmpadas",
        getText("IGUI_PB_Appliances") or "Eletrodomésticos",
        getText("IGUI_PB_TargetTemp") or "Ajustar Temperatura",
    }
    ...
    local sampleValues = {
        ...
        "0.288 " .. (getText("IGUI_PB_LitresPerHour") or "Litros por hora"),
        "~1" .. (getText("IGUI_PB_Days") or " dias") .. " 9" .. (getText("IGUI_PB_Hours") or " horas")
    }
...
function PB_GeneratorInfoWindow:drawStatBar(x, y, label, value, maxValue, greenHigh, barCol)
    value      = value or 0
...
function PB_GeneratorInfoWindow:drawStrainBar(x, y, label, strain, barCol)
    strain         = strain or 0
...
function PB_GeneratorInfoWindow:drawBarrelFuelBar(x, y, amount, maxAmount, barWidth)
    amount             = amount or 0
    ...
    local PR, PG, PB   = 0.97, 0.93, 0.55 -- cor do segmento cheio
    self._strainSegTex = self._strainSegTex or getTexture(STRAIN_SEG_PATH)
    ...
    for i = 1, SEG_COUNT do
        ...
        if i <= fullSegs then
            if self._strainSegTex then
                self:drawTextureScaled(self._strainSegTex, sx, by, segW, BAR_H, 0.95, PR, PG, PB)
            else
                self:drawRect(sx, by, segW, BAR_H, 0.95, PR, PG, PB)
            end
        elseif i == fullSegs + 1 and partial > 0 then
            local partW = math.max(2, math.floor(segW * partial))
            if self._strainSegTex then
                self:drawTextureScaled(self._strainSegTex, sx, by, partW, BAR_H, 0.95, PR, PG, PB)
            else
                self:drawRect(sx, by, partW, BAR_H, 0.95, PR, PG, PB)
            end
        end
    end
end
...
function PB_GeneratorInfoWindow:getGeneratorIcon(gen)
...
local function GetGeneratorStateData(x, y, z)
    local SM = PoweredBuildings and PoweredBuildings.Core and PoweredBuildings.Core.StateManager
    local GeneratorData = PoweredBuildings and PoweredBuildings.Data and PoweredBuildings.Data.Generator
...
local function ResolveBuildingDataFromAnchor(anchorSquare, buildingHint)
    ...
    local bldId = PoweredBuildings.Data.Building.MakeId(ax, ay, az)
...
function PB_GeneratorInfoWindow:scanAllGenerators()
...
function PB_GeneratorInfoWindow:sortGeneratorsByConnectionOrder(genList)
    local order = {}
    local SM = PoweredBuildings and PoweredBuildings.Core and PoweredBuildings.Core.StateManager
...
function PB_GeneratorInfoWindow:updateData()
    if not self.generator then return end
    ...
    local Barrels_ud               = PoweredBuildings.Fuel and PoweredBuildings.Fuel.Barrels
...
function PB_GeneratorInfoWindow:onMouseDown(x, y)
...
function PB_GeneratorInfoWindow:onMouseMove(dx, dy)
...
function PB_GeneratorInfoWindow:onMouseMoveOutside(dx, dy)
...
local function SendHeatingConfig(window, enabled, targetTemp)
    ...
    sendClientCommand("PoweredBuildings", "HeatingToggle", payload)
end
...
local function ApplyHeatingConfigLocal(window, enabled, targetTemp)
    ...
    local StateManager = PoweredBuildings and PoweredBuildings.Core and PoweredBuildings.Core.StateManager
    local GeneratorData = PoweredBuildings and PoweredBuildings.Data and PoweredBuildings.Data.Generator
    local BuildingData = PoweredBuildings and PoweredBuildings.Data and PoweredBuildings.Data.Building
    ...
    if md then
        ...
        local runtime = PoweredBuildings and PoweredBuildings.Core and PoweredBuildings.Core.Runtime
...
function PB_GeneratorInfoWindow:onHeatingSetState(enable)
    if not self.generator then return end
    ...
    if PB_HeatingClient then
        ...
        if sq then
            ...
            if enable and liveHeatingGenerator and liveHeatingGenerator:isActivated() then
                PB_HeatingClient.Apply(liveHeatingGenerator)
            else
                PB_HeatingClient.Remove(key)
            end
        end
    end
    self:requestFreshStats()
...
function PB_GeneratorInfoWindow:onHeatingToggle()
    self:onHeatingSetState(not (self._heatingEnabled == true))
end

function PB_GeneratorInfoWindow:onHeatingTempChange(delta)
    if not self.generator then return end
    local Runtime       = PoweredBuildings.Core and PoweredBuildings.Core.Runtime
    local isMPClient    = Runtime and Runtime.IsMultiplayerClient and Runtime.IsMultiplayerClient()
    local liveGenerator = GetLiveGeneratorObject(self.generator)
    ...
    if PB_HeatingClient and md and md.HeatingEnabled == true then
        local sq = liveHeatingGenerator and liveHeatingGenerator:getSquare() or nil
        if sq then
            local key = sq:getX() .. "_" .. sq:getY() .. "_" .. sq:getZ()
            PB_HeatingClient.Remove(key)
            if liveHeatingGenerator and liveHeatingGenerator:isActivated() then
                PB_HeatingClient.Apply(
...
function PB_GeneratorInfoWindow:requestFreshStats()
    if not self.generator then return end
    local md = self.generator:getModData()
    ...
    local SM_ui = PoweredBuildings and PoweredBuildings.Core and PoweredBuildings.Core.StateManager
    ...
    if isClient() then
        sendClientCommand("PoweredBuildings", "ForceDist", { buildingID = buildingID })
    else
        local Dist = PoweredBuildings and PoweredBuildings.Power and PoweredBuildings.Power.Distributor
        if Dist then
            ...
            PoweredBuildings.Core.Logger.Warn("requestFreshStats: Distributor indisponível", "UI")
        end

        local FuelMgr = PoweredBuildings and PoweredBuildings.Fuel and PoweredBuildings.Fuel.Manager
...
function PB_GeneratorInfoWindow:findBuildingDef()
...
function PB_GeneratorInfoWindow:applyHighlights(enabled)
...
function PB_GeneratorInfoWindow:onToggleCoverage()
...
function PB_GeneratorInfoWindow:onClose()
    self:close()
end
...
function PB_GeneratorInfoWindow:render()
    ...
    y                   = self:drawSection(x0, y, getText("IGUI_PB_SectionGenerator") or "GERADOR")
    ...
    for idx, gen in ipairs(self.allGenerators) do
        ...
        local statusText = gActive and (getText("IGUI_PB_On") or "LIGADO") or (getText("IGUI_PB_Off") or "DESLIGADO")
        ...
        local fuelText = (getText("IGUI_PB_Fuel") or "Combustível") .. ": " .. string.format("%3.0f%%", fPct)
        ...
        local condText = (getText("IGUI_PB_Condition") or "Condição") .. ": " .. string.format("%3.0f%%", gCond)
    ...
    if poolMaxFuel > 0 then
        ...
        if not iconTex then iconTex = getTexture("media/textures/Item_PetrolCan.png") end
        if iconTex then
        ...
        if #barrelList == 0 then
            ...
            for bidx, bdata in ipairs(barrelList) do
                ...
                if self._barrelIconCache[spriteKey] ~= nil then
                    ...
                    if not iconTex then iconTex = getTexture("media/textures/Item_Petrol.png") end
    ...
    y = self:drawSection(x0, y, getText("IGUI_PB_SectionBuilding") or "CONSTRUÇÃO")

    if d.buildingID then
        if d.isPowered then
            self:drawText("[+] " .. (getText("IGUI_PB_Powered") or "Energizado"), x0, y + 5, 0.20, 0.90, 0.28, 1, FONT_S)
        else
            self:drawText("[-] " .. (getText("IGUI_PB_Unpowered") or "Sem Energia"), x0, y + 5, 0.85, 0.48, 0.15, 1,
                FONT_S)
        end
        y = y + LINE_H + 2

        local drawLabel = getText("IGUI_PB_Consumers") or "Total"
        ...
        drawLabel = getText("IGUI_PB_Appliances") or "Eletrodomésticos"
        drawLabel = string.gsub(drawLabel, ":%s*$", "")
        self:drawText(drawLabel .. ":", x0 + 8, y + 5, 1, 1, 1, 0.60, FONT_S)
        ...
        if d.powerDraw > 0 then
            drawLabel = getText("IGUI_PB_PowerDraw") or "Consumo Elétrico"
            drawLabel = string.gsub(drawLabel, ":%s*$", "")
            self:drawText(drawLabel .. ":", x0, y + 5, 1, 1, 1, 0.80, FONT_S)

            local unitsStr = getText("IGUI_PB_UnitsPerGen") or "unidades por gerador"
            self:drawText(string.format("%.1f " .. unitsStr, d.powerDraw), x0 + barCol, y + 5, 0.78, 0.95, 0.78, 1,
                FONT_S)
            y = y + LINE_H

            drawLabel = getText("IGUI_PB_FuelRate") or "Consumo de Combustível"
            drawLabel = string.gsub(drawLabel, ":%s*$", "")
            self:drawText(drawLabel .. ":", x0, y + 5, 1, 1, 1, 0.80, FONT_S)
            local lphStr = getText("IGUI_PB_LitresPerHour") or "Litros por hora"
            ...
            if rtFuel > 0 and actualLph > 0 then
                ...
                local dayStr  = getText("IGUI_PB_Days") or " dias"
                local hourStr = getText("IGUI_PB_Hours") or " horas"
...
function PB_GeneratorInfoWindow:close()
    self:applyHighlights(false)
    if self.generator then
        ...
        if ok and gx then
            ...
            PB_GeneratorInfoWindow.instances[genKey] = nil
        else
            for k, v in pairs(PB_GeneratorInfoWindow.instances) do
                if v == self then
                    PB_GeneratorInfoWindow.instances[k] = nil; break
                end
            end
        end
    end
...
function PB_GeneratorInfoWindow:new(x, y, generator, playerNum, anchorSquare)
...
function PB_GeneratorInfoWindow.Open(character, generator, anchorSquare, buildingHint)
    ...
    local existing = PB_GeneratorInfoWindow.instances[genKey]
    if existing then
    ...
    local win = PB_GeneratorInfoWindow:new(x, y, generator, character:getPlayerNum(), anchorSquare)
    ...
    PB_GeneratorInfoWindow.instances[genKey] = win
    win:requestFreshStats()
    PoweredBuildings.Print("[GeneratorInfoWindow] Aberta para o gerador em " .. genKey)
end

function PB_GeneratorInfoWindow.CloseAll()
    for _, win in pairs(PB_GeneratorInfoWindow.instances) do win:close() end
end
