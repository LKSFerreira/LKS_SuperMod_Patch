...
if not PoweredBuildings then
    print("[LKS PATCH - PB_UI_DebugPanel.lua] namespace PoweredBuildings não encontrado - pulando carregamento do módulo")
    return
end

print("[LKS PATCH - PB_UI_DebugPanel.lua] Carregando Painel de Depuração de Interface...")

require "ISUI/ISCollapsableWindow"
require "ISUI/ISButton"

PoweredBuildings.RegisterModule("PB_UI_DebugPanel")
PoweredBuildings.UI = PoweredBuildings.UI or {}

-- ============================================================================
-- ⚙️ CONFIGURAÇÕES DE DIMENSÕES E ASSETS (PADRÃO DE NOMENCLATURA PB)
-- ============================================================================
...
-- CLASSE
-- ============================================================================
local PB_DebugPanel = ISCollapsableWindow:derive("PB_DebugPanel")

-- ============================================================================
-- CICLO DE VIDA DA INTERFACE
-- ============================================================================

function PB_DebugPanel:initialise()
    ISCollapsableWindow.initialise(self)
end

function PB_DebugPanel:createChildren()
    ...
    self.btnIsoRegions = self:addButtonAt("Mapa IsoRegions", halfX, halfW, y, function()
        if IsoRegionsWindow then
            ...
            print("[LKS PATCH - PB_UI_DebugPanel.lua] Janela IsoRegionsWindow não está disponível no cliente")
        end
    end)
...
function PB_DebugPanel:addButtonAt(text, x, w, y, onClick)
...
function PB_DebugPanel:close()
    self:removeFromUIManager()
    PB_DebugPanel.instance = nil
end

function PB_DebugPanel:new(x, y)
    local o = ISCollapsableWindow:new(x, y, WIN_W, WIN_H)
    setmetatable(o, self)
    self.__index = self
    o.title = "Painel de Depuração PB"
...
function PB_DebugPanel.Toggle()
    ...
    if PB_DebugPanel.instance then
        PB_DebugPanel.instance:close()
        PB_DebugPanel.instance = nil
    else
        local x = getCore():getScreenWidth() / 2 - WIN_W / 2
        local y = 50
        PB_DebugPanel.instance = PB_DebugPanel:new(x, y)
        PB_DebugPanel.instance:initialise()
        PB_DebugPanel.instance:addToUIManager()
    end
end
...
local function OnKeyPressed(key)
    ...
    if key == 12 or key == 74 then
        PB_DebugPanel.Toggle()
    end
end

Events.OnKeyPressed.Add(OnKeyPressed)

print("[LKS PATCH - PB_UI_DebugPanel.lua] Interface de Debug carregada com sucesso!")
print("[LKS PATCH - PB_UI_DebugPanel.lua] Use a tecla '-' (Teclado principal ou NumPad) para abrir/fechar (Apenas Admins/Modo Debug).")