...
- **Correção e Melhoria das Máquinas de Lavar e Secar (em progresso)**: Motor de ciclo Lua complementar (Java não processa itens). Sistema de condição da máquina, elegibilidade expandida (sapatos, bolsas, mochilas), penalidade por uso inadequado, Ciclo Automático para Combo. Plano em `.metadocs/implementation_plans/`. Pesquisa em `docs/pesquisa_lavadoras_secadoras.md`.
...
- [x] **Fagocitação Light My Fire (20/06/2026)**: Incorporação nativa do mod de Eurymachus. Handler unificado `LKS_Menu_Fire_FuelSolid.lua`, patch de combustível com handlers próprios, menu hierárquico de ignição, traduções PT-BR.
- [x] **Organizador de Itens (23/06/2026)**: Sistema de favoritar containers e guardar itens automaticamente. Scan 50×50 multi-andar, ISWalkToTimedAction + ISInventoryTransferAction, overlay de estrela, bags no chão. 4 arquivos Lua + traduções.
- [x] **Reformulação do Combustível Sólido (24/06/2026)**: Classificador invertido, agrupamento por categoria, handler Loot Window in-container, AutoBurn com penalidade 75%, penalidade de umidade, processamento recursivo. 3 arquivos Lua novos.
