...
Events.OnFillWorldObjectContextMenu.Add(function(playerNum, context, worldobjects, test)
    ...
    if linked then
        ...
        local opt = context:addOption(
            getText("IGUI_PB_UnlinkBarrel") or "Unlink from Fuel Pool",
            worldobjects,
            function()
        ...
        -- Link option
        local opt = context:addOption(
            getText("IGUI_PB_LinkBarrel") or "Link to Fuel Pool",
            worldobjects,
            function()
                if not canLink then
                    player:Say(getText("IGUI_PB_BarrelNoBuildingNearby") or "No powered building nearby")
                    return
                end
                ISTimedActionQueue.add(
        ...
        if not canLink then
            ...
            tip:setName(getText("IGUI_PB_BarrelNoBuildingNearby") or "No powered building nearby")
            opt.toolTip = tip
        end
    end
end)
